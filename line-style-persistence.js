...
/**
 * Validates line style settings before saving
 */
export function validateLineStyleSettings(settings) {
  const validStyles = ['solid', 'dashed', 'dotted', 'dash-dot'];
  const errors = [];

  // Validate family line settings
  if (settings.familyLineStyle && !validStyles.includes(settings.familyLineStyle)) {
    errors.push('Invalid family line style');
  }
  if (settings.familyLineThickness && (settings.familyLineThickness < 1 || settings.familyLineThickness > 10)) {
    errors.push('Family line thickness must be between 1 and 10');
  }

  // Validate spouse line settings
  if (settings.spouseLineStyle && !validStyles.includes(settings.spouseLineStyle)) {
    errors.push('Invalid spouse line style');
  }
  if (settings.spouseLineThickness && (settings.spouseLineThickness < 1 || settings.spouseLineThickness > 10)) {
    errors.push('Spouse line thickness must be between 1 and 10');
  }

  // Validate line-only settings
  if (settings.lineOnlyStyle && !validStyles.includes(settings.lineOnlyStyle)) {
    errors.push('Invalid line-only style');
  }
  if (settings.lineOnlyThickness && (settings.lineOnlyThickness < 1 || settings.lineOnlyThickness > 10)) {
    errors.push('Line-only thickness must be between 1 and 10');
  }
...
/**
 * Applies default line style settings to a canvas renderer
 */
export function applyDefaultLineStyles(canvasRenderer) {
  if (!canvasRenderer || !canvasRenderer.settings) {
    return;
  }

  // Apply default family line settings
  canvasRenderer.settings.familyLineStyle = canvasRenderer.settings.familyLineStyle || 'solid';
  canvasRenderer.settings.familyLineThickness = canvasRenderer.settings.familyLineThickness || 2;
  canvasRenderer.settings.familyLineColor = canvasRenderer.settings.familyLineColor || '#7f8c8d';

  // Apply default spouse line settings
  canvasRenderer.settings.spouseLineStyle = canvasRenderer.settings.spouseLineStyle || 'dashed';
  canvasRenderer.settings.spouseLineThickness = canvasRenderer.settings.spouseLineThickness || 2;
  canvasRenderer.settings.spouseLineColor = canvasRenderer.settings.spouseLineColor || '#e74c3c';

  // Apply default line-only settings
  canvasRenderer.settings.lineOnlyStyle = canvasRenderer.settings.lineOnlyStyle || 'dash-dot';
  canvasRenderer.settings.lineOnlyThickness = canvasRenderer.settings.lineOnlyThickness || 2;
  canvasRenderer.settings.lineOnlyColor = canvasRenderer.settings.lineOnlyColor || '#9b59b6';

  console.log('✅ Default line style settings applied');
}